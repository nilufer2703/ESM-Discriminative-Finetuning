...
def train_eval(dataset, model, split, device, mean, mut_mean, batch_size, flip, lr, kernel_size, input_size, dropout, alpha, gb1_shorten): 
    ...
    if model.startswith('esm'): 
        ...
        # evaluate
        train_rho, train_mse, train_pearson, train_top1, train_top100, train_top500 = evaluate_esm(train_iterator, esm_linear, device, len(train_data), mean, mut_mean, EVAL_PATH / 'test')
        test_rho, test_mse, test_pearson, test_top1, test_top100, test_top500 = evaluate_esm(test_iterator, esm_linear, device, len(test_data), mean, mut_mean, EVAL_PATH / 'train')        
    ...
    print('train stats: Spearman: %.2f MSE: %.2f Pearson: %.2f top1: %.2f top100: %.2f top500: %.2f' % (train_rho, train_mse, train_pearson, train_top1, train_top100, train_top500))
    print('test stats: Spearman: %.2f MSE: %.2f Pearson: %.2f top1: %.2f top100: %.2f top500: %.2f' % (test_rho, test_mse, test_pearson, test_top1, test_top100, test_top500))
