...
def topk_retrieval(labels, preds, k=10):
    """
    Top-k Retrieval

    labels : true fitness
    preds  : predicted fitness
    """

    true_top = set(np.argsort(labels)[-k:])
    pred_top = set(np.argsort(preds)[-k:])

    hits = len(true_top.intersection(pred_top))
...
def evaluate_esm(data_iterator, model, device, size, mean, mut_mean, SAVE_PATH):
    ...
    pearson = pearsonr(labels, out)[0]
    top10 = topk_retrieval(
        labels,
        out,
        k=10
    )

    top100 = topk_retrieval(
        labels,
        out,
        k=100
    )

    top500 = topk_retrieval(
        labels,
        out,
        k=500
    )

    return rho, mse, pearson, top10, top100, top500
